  ...
  return (
    <Layout title="Payroll" activeTab="payroll">
      ...
      <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4 md:p-6 mb-4">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div class="flex items-end gap-2">
            <button
              ...
              class="w-full bg-purple-600 text-white px-4 py-2.5 min-h-[44px] rounded-lg font-medium hover:bg-purple-700"
            >
        ...
        <p id="success-general" class="text-sm text-green-600 mt-3 p-3 bg-green-50 rounded-lg hidden"></p>
      </div>
          ...
          <table class="w-full min-w-[800px]">
            <thead class="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Thưởng Chất lượng</th>
                <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Điểm ORS</th>
      ...
      <script dangerouslySetInnerHTML={{
        ...
        async function loadPayrollData() {
          ...
          document.getElementById('success-general')?.classList.add('hidden');
          ...
          try {
            const btn = document.getElementById('btn-load-payroll');
            ...
            // Update stats
            const totalBonus = payrollData.reduce((sum, d) => sum + (d.kpi_bonus || 0) + (d.kpi_allowance || 0), 0);
            ...
            if (payrollData.length === 0) {
               ...
               errorEl.textContent = 'Chưa có dữ liệu cho kỳ lương này. Vui lòng bấm "Tính toán dữ liệu" để hệ thống tổng hợp.';
            ...
            const btn = document.getElementById('btn-load-payroll');
            btn.disabled = false;
            btn.innerHTML = '<i class="fas fa-download mr-2"></i>Tải dữ liệu';
        ...
        async function calculatePayrollData() {
          const warehouseCode = document.getElementById('warehouseCode').value;
          const payrollPeriod = document.getElementById('payrollPeriod').value;
          ...
          if (!confirm('Bạn có chắc chắn muốn chạy tính toán lại dữ liệu KPI & Lương cho tháng ' + payrollPeriod + '? Quá trình này có thể mất vài phút.')) {
            return;
          }
          ...
          btn.disabled = true;
          btn.innerHTML = '<i class="fas fa-spinner fa-spin mr-2"></i>Đang tính...';

          document.getElementById('error-general')?.classList.add('hidden');
          document.getElementById('success-general')?.classList.add('hidden');

          try {
            // Run Monthly Pipeline (Job A -> B -> C -> D)
            // This ensures all weekly data is calculated before monthly aggregation
            await axios.post(window.API_BASE + '/jobs/run-monthly', {
              payrollPeriod,
              warehouseCode: warehouseCode || undefined
            });

            const successEl = document.getElementById('success-general');
            successEl.textContent = 'Tính toán dữ liệu hoàn tất! Đang tải lại danh sách...';
            ...
            const errorEl = document.getElementById('error-general');
            errorEl.textContent = 'Lỗi khi tính toán dữ liệu: ' + (error.response?.data?.error || error.message);
            errorEl.classList.remove('hidden');
          } finally {
            btn.disabled = false;
        ...
        function renderTable(data) {
          document.getElementById('payroll-data-body').innerHTML = data.map(d => \`
            <tr class="hover:bg-gray-50">
              ...
              <td class="px-4 py-3 font-medium cursor-pointer text-blue-600 hover:text-blue-800" onclick="window.open('/employee?staffId=\${d.staff_id}&warehouseCode=\${d.warehouse_code}', '_blank')">\${d.staff_id}</td>
              ...
              <td class="px-4 py-3 font-medium text-blue-600">\${d.kpi_allowance?.toLocaleString() || 0}</td>
              <td class="px-4 py-3 text-orange-600">\${d.penalty?.toLocaleString() || 0} pts</td>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
              <div class="flex items-start justify-between gap-3">
                <div class="flex items-center gap-3">
                  ...
                  <div>
                    <p class="font-bold text-gray-900 cursor-pointer text-blue-600" onclick="window.open('/employee?staffId=\${d.staff_id}&warehouseCode=\${d.warehouse_code}', '_blank')">\${d.staff_id}</p>
              ...
              <div class="mt-3 grid grid-cols-2 gap-2 text-sm">
                ...
                <div><span class="text-gray-500">Thưởng KPI:</span> <span class="font-bold text-green-600">\${d.kpi_bonus?.toLocaleString() || 0}</span></div>
                <div><span class="text-gray-500">Thưởng CL:</span> <span class="font-medium text-blue-600">\${d.kpi_allowance?.toLocaleString() || 0}</span></div>
                <div><span class="text-gray-500">Điểm ORS:</span> <span class="text-orange-600">\${d.penalty?.toLocaleString() || 0} pts</span></div>
              </div>
        ...
        function renderWarehouseSummary(data) {
          ...
          data.forEach(d => {
            ...
            summary[d.warehouse_code].totalBonus += (d.kpi_bonus || 0) + (d.kpi_allowance || 0);
      ...
      const errorEl = document.getElementById('success-general');
      ...
      window.calculatePayrollData = calculatePayrollData;
