-- ============================================================
-- Perfomance Management System - DATABASE SCHEMA
-- Version: 1.0
-- Date: 2025-01-19
-- ============================================================
...
CREATE TABLE IF NOT EXISTS role_main_task_config (
    ...
    role_id INTEGER,
    main_task VARCHAR(50) NOT NULL CHECK (
        main_task IN (
            'packing',
            'picking',
            'handover',
...
CREATE INDEX idx_role_main_task_active ON role_main_task_config (
    is_active,
    effective_from,
    effective_to
);
...
CREATE INDEX idx_ranking_range_active ON ranking_range_config (
    is_active,
    effective_from,
    effective_to
);
...
CREATE TABLE IF NOT EXISTS kpi_bonus_config (
    ...
    country VARCHAR(10),
    ranking_score_min INTEGER NOT NULL CHECK (
        ranking_score_min BETWEEN 1 AND 5
    ),
    ranking_score_max INTEGER NOT NULL CHECK (
        ranking_score_max BETWEEN 1 AND 5
    ),
    calculation_type VARCHAR(20) NOT NULL CHECK (
        calculation_type IN ('PER_POINT', 'FIXED')
    ),
    ...
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW(),
    CONSTRAINT valid_ranking_range CHECK (
        ranking_score_min <= ranking_score_max
    )
);
...
CREATE INDEX idx_kpi_bonus_active ON kpi_bonus_config (
    is_active,
    effective_from,
    effective_to
);
...
CREATE TABLE IF NOT EXISTS ors_catalog (
    ...
    job_group VARCHAR(50) NOT NULL CHECK (
        job_group IN (
            'packing',
            'picking',
            'handover',
    ...
    description TEXT,
    severity_level VARCHAR(5) NOT NULL CHECK (
        severity_level IN ('S1', 'S2', 'S3', 'S4', 'S5')
    ),
...
CREATE TABLE IF NOT EXISTS ors_milestone_config (
    ...
    milestone_level VARCHAR(20) NOT NULL CHECK (
        milestone_level IN (
            'GREEN',
            'YELLOW',
            'ORANGE',
    ...
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW(),
    CONSTRAINT valid_points_range CHECK (
        ors_points_from <= ors_points_to
    )
);
...
CREATE TABLE IF NOT EXISTS kpi_weekly_summary (
    ...
    data_status VARCHAR(30) DEFAULT 'OK' CHECK (
        data_status IN (
            'OK',
            'INSUFFICIENT_HOURS',
            'NO_DATA',
    ...
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW(),
    CONSTRAINT uq_kpi_weekly_staff UNIQUE (
        warehouse_code,
        staff_id,
        year_week
    )
...
CREATE TABLE IF NOT EXISTS ranking_weekly_result (
    ...
    status VARCHAR(30) DEFAULT 'FINAL' CHECK (
        status IN (
            'FINAL',
            'INSUFFICIENT_DATA',
            'PENDING_REVIEW',
    ...
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW(),
    CONSTRAINT uq_ranking_weekly_staff UNIQUE (
        warehouse_code,
        staff_id,
        year_week
    )
...
CREATE TABLE IF NOT EXISTS ors_monthly_summary (
    ...
    milestone_level VARCHAR(20) DEFAULT 'GREEN' CHECK (
        milestone_level IN (
            'GREEN',
            'YELLOW',
            'ORANGE',
    ...
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW(),
    CONSTRAINT uq_ors_monthly_staff UNIQUE (
        warehouse_code,
        staff_id,
        payroll_period
    )
...
CREATE TABLE IF NOT EXISTS kpi_monthly_summary (
    ...
    final_ranking_score INTEGER NOT NULL DEFAULT 3 CHECK (
        final_ranking_score BETWEEN 1 AND 5
    ),
    ...
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW(),
    CONSTRAINT uq_kpi_monthly_staff UNIQUE (
        warehouse_code,
        staff_id,
        payroll_period
    )
...
CREATE TABLE IF NOT EXISTS payroll_kpi_bridge (
    ...
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW(),
    CONSTRAINT uq_payroll_bridge_staff UNIQUE (
        warehouse_code,
        payroll_period,
        staff_id
    )
...
VALUES
    ...
    (
        'PACK-001',
        'packing',
        'Đóng gói thiếu/sai vật liệu chèn lót',
        'Không sử dụng đủ giấy chèn/xốp nổ cho hàng dễ vỡ hoặc hàng điện tử, gây nguy cơ bể vỡ trong vận chuyển',
        'S2',
        3
    ),
    (
        'PACK-002',
        'packing',
        'Không seal chất lỏng/mỹ phẩm',
        'Không chèn/quấn vòi, nắp chai lọ mỹ phẩm, chất lỏng dẫn đến rò rỉ',
        'S3',
        6
    ),
    (
        'PACK-003',
        'packing',
        'Sử dụng hộp cũ/hư hỏng',
        'Sử dụng hộp đã qua sử dụng, móp méo, không có logo theo quy định',
        'S1',
        2
    ),
    (
        'PACK-004',
        'packing',
        'Đóng gói sai hàng/thiếu hàng',
        'Đóng gói không đúng sản phẩm hoặc số lượng so với đơn hàng, dẫn đến đơn bị hoàn/khiếu nại',
        'S3',
        8
    ),
    (
        'PACK-005',
        'packing',
        'Trễ cam kết đóng gói (SLA)',
        'Không hoàn thành đóng gói trong thời gian quy định (2h với đơn ưu tiên/B2B, 6h với đơn thường)',
        'S2',
        4
    ),
...
(
    'PICK-001',
    'picking',
    'Lấy sai sản phẩm (Sai SKU)',
    'Lấy nhầm sản phẩm do không kiểm tra kỹ Barcode/SKU trước khi lấy',
    'S2',
    5
),
(
    'PICK-002',
    'picking',
    'Báo cáo thiếu hàng sai (Hàng ảo)',
    'Xác nhận thiếu hàng trên hệ thống nhưng thực tế hàng vẫn còn ở vị trí khác hoặc bị che khuất',
    'S3',
    7
),
(
    'PICK-003',
    'picking',
    'Lấy hàng cận date/hết date sai chiến lược',
    'Không tuân thủ FEFO (First Expired First Out), lấy hàng mới nhập thay vì hàng cũ cần xuất trước',
    'S3',
    6
),
...
(
    'HAND-001',
    'handover',
    'Bàn giao sai hãng vận chuyển',
    'Phân loại sai hãng vận chuyển, đưa nhầm hàng cho bưu tá/tài xế khác',
    'S3',
    6
),
(
    'HAND-002',
    'handover',
    'Thiếu biên bản/chữ ký bàn giao (POD)',
    'Bàn giao hàng nhưng không có biên bản hoặc chữ ký xác nhận của tài xế/bưu tá',
    'S3',
    8
),
(
    'HAND-003',
    'handover',
    'Bàn giao trễ (Miss cutoff time)',
    'Không kịp bàn giao đơn hàng trong khung giờ quy định (trước 20h hoặc 12h hôm sau)',
    'S2',
    5
),
...
(
    'PUT-001',
    'putaway',
    'Để hàng trực tiếp dưới sàn',
    'Vi phạm quy chuẩn lưu kho - đặt hàng hóa trực tiếp xuống sàn mà không có pallet/kệ',
    'S3',
    7
),
(
    'PUT-002',
    'putaway',
    'Lưu kho chung lô hạn sử dụng khác nhau',
    'Để lẫn lộn các hàng hóa có hạn sử dụng (Date) khác nhau vào cùng một vị trí',
    'S2',
    4
),
(
    'PUT-003',
    'putaway',
    'Sắp xếp sai chiều quy định',
    'Đặt hàng sai chiều (ngược/ngang) gây nguy cơ chảy nước hoặc hư hỏng sản phẩm',
    'S2',
    3
),
(
    'PUT-004',
    'putaway',
    'Không tháo bao bì bảo quản tạm',
    'Không tuân thủ quy chuẩn bọc plastic/seal sách hoặc quần áo trước khi lên kệ lưu trữ',
    'S1',
    2
),
...
(
    'INSP-001',
    'inspection',
    'Bỏ sót lỗi ngoại quan (QC sai)',
    'Nhập kho hàng móp méo, hư hỏng, ẩm ướt nhưng không phát hiện và phân loại Grade D',
    'S3',
    6
),
(
    'INSP-002',
    'inspection',
    'Nhập hàng cận date không cảnh báo',
    'Nhập kho hàng có hạn sử dụng dưới 6 tháng mà không có xác nhận của người bán',
    'S3',
    7
),
(
    'INSP-003',
    'inspection',
    'Không kiểm tra niêm phong/Seal',
    'Đối với hàng giá trị cao/điện tử, không kiểm tra tình trạng nguyên seal/tem nhãn trước khi nhập',
    'S4',
    12
),
...
(
    'COIN-001',
    'co_inspection',
    'Thiếu bằng chứng đồng kiểm',
    'Không chụp ảnh/quay video quá trình đồng kiểm, đặc biệt với hàng hoàn/nhập kho',
    'S3',
    8
),
(
    'COIN-002',
    'co_inspection',
    'Ký nhận sai số lượng thực tế',
    'Xác nhận trên biên bản số lượng khác với thực tế nhận được từ đối tác',
    'S3',
    9
),
...
(
    'DATA-001',
    'data_entry',
    'Nhập sai kích thước/trọng lượng',
    'Nhập liệu sai cân nặng, kích thước (DIM) dẫn đến tính sai phí vận chuyển/lưu kho',
    'S3',
    6
),
(
    'DATA-002',
    'data_entry',
    'Nhập sai hạn sử dụng (Expiry Date)',
    'Cập nhật sai hạn sử dụng lên hệ thống WMS, gây rủi ro xuất hàng hết hạn',
    'S3',
    7
),
(
    'DATA-003',
    'data_entry',
    'Sai thông tin kiện hàng',
    'Nhập sai số lượng kiện hoặc mã kiện trong lô hàng nhập',
    'S2',
    4
),
...
(
    'UNLD-001',
    'unloading',
    'Làm hư hỏng hàng khi hạ tải',
    'Thao tác ẩu, làm rơi vỡ hàng hóa trong quá trình dỡ hàng từ xe tải',
    'S3',
    8
),
(
    ...
    'Bê vác tay đối với hàng nặng/cồng kềnh thay vì dùng xe nâng, gây rủi ro an toàn và hỏng hàng',
    'S4',
    15
),
...
(
    'MOVE-001',
    'move',
    'Vận chuyển hàng không che chắn/cố định',
    'Không quấn PE hoặc cố định hàng trên pallet khi di chuyển, gây xô lệch, rơi vỡ',
    'S2',
    5
),
(
    'MOVE-002',
    'move',
    'Làm rơi/móp méo hàng nội bộ',
    'Gây hư hỏng hàng hóa trong quá trình di chuyển vị trí trong kho',
    'S3',
    6
),
...
(
    'CYCL-001',
    'cycle_count',
    'Sai lệch số lượng kiểm kê',
    'Báo cáo số lượng tồn kho sai so với thực tế tại vị trí kiểm',
    'S3',
    7
),
(
    ...
    'cycle_count',
    'Bỏ sót vị trí/SKU',
    'Không kiểm đếm hết các sản phẩm trong vị trí được chỉ định',
    'S2',
    4
),
...
(
    'RLAB-001',
    'relabel',
    'Dán đè thông tin gốc/nhãn gốc',
    'Dán tem phụ che khuất nội dung bắt buộc hoặc che nhãn gốc của sản phẩm',
    'S3',
    8
),
(
    'RLAB-002',
    'relabel',
    'Dán sai vị trí quy định',
    'Dán tem không đúng vị trí đã được hướng dẫn hoặc thống nhất với khách hàng',
    'S1',
    2
),
(
    'RLAB-003',
    'relabel',
    'Không dán nhãn Combo/Set',
    'Không dán nhãn "Không tách lẻ" cho hàng bộ/combo, gây sai sót khi xuất kho',
    'S3',
    7
),
(
    'RLAB-004',
    'relabel',
    'Tem nhãn mờ/nhăn/rách',
    'Tem in bị mờ không đọc được hoặc dán bị nhăn/gấp nếp không đạt chuẩn',
    'S1',
    2
) ON CONFLICT (ors_code) DO
UPDATE
SET
    name = EXCLUDED.name,
...
VALUES (
        ...
        'NO_ACTION',
        0.0000,
        'Bình thường - Tiếp tục làm việc tốt'
    ),
    (
        NULL,
        10,
        ...
        'WARNING',
        0.0000,
        'Cảnh báo - Gửi thông báo cảnh báo, quản lý nhận alert để coaching'
    ),
    (
        NULL,
        20,
        ...
        'TRAINING_REQUIRED',
        0.1000,
        'Cần cải thiện - Yêu cầu đào tạo lại, giảm 10% KPI bonus'
    ),
    (
        NULL,
        30,
        ...
        'HR_REVIEW',
        0.3000,
        'Nghiêm trọng - Yêu cầu đánh giá HR, giảm 30% KPI bonus'
    ),
    (
        NULL,
        40,
        ...
        'DISCIPLINARY',
        1.0000,
        'Đặc biệt nghiêm trọng - Mất toàn bộ KPI bonus, xem xét kỷ luật'
...
VALUES (
        ...
        5,
        20,
        'Xuất sắc - Vượt trội, năng suất cao nhất'
    ),
    (
        NULL,
        NULL,
    ...
    (
        ...
        3,
        20,
        'Đạt yêu cầu - Hoàn thành công việc theo chuẩn'
    ),
    (
        NULL,
        NULL,
        ...
        2,
        20,
        'Cần cải thiện - Chưa đạt kỳ vọng, cần hỗ trợ'
    ),
    (
        NULL,
        NULL,
        ...
        1,
        20,
        'Chưa đạt - Cần can thiệp và đào tạo lại'
    ) ON CONFLICT DO NOTHING;
